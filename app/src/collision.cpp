...
Collision CheckCollision(Ball& one, Entity& two)  // AABB - Circle collision
{
    ...
    // Now that we know the the clamped values, add this to AABB_center and we
    // get the value of box closest to circle
    ...
    if (glm::length(difference)
        < one.Radius)  // not <= since in that case a collision also occurs when
                       // object one exactly touches object two, which they are
