...
int main(void)
{
    ...
    window
        = glfwCreateWindow(1024, 768, "Tutorial 18 - Billboards", NULL, NULL);
    if (window == NULL)
    {
        fprintf(
            stderr,
    ...
    GLuint programID = LoadShaders(
        R"(C:\Dev\tobanteGaming\projects\template-opengl\resources\3d-tutorial\Billboard.vertexshader)",
        R"(C:\Dev\tobanteGaming\projects\template-opengl\resources\3d-tutorial\Billboard.fragmentshader)");
    ...
    GLuint Texture = loadDDS(
        R"(C:\Dev\tobanteGaming\projects\template-opengl\resources\3d-tutorial\ExampleBillboard.DDS)");
    ...
    GLuint cubeProgramID = LoadShaders(
        R"(C:\Dev\tobanteGaming\projects\template-opengl\resources\shaders\TransformVertexShader.vertexshader)",
        R"(C:\Dev\tobanteGaming\projects\template-opengl\resources\shaders\TextureFragmentShader.fragmentshader)");
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(g_cube_vertex_buffer_data),
                 g_cube_vertex_buffer_data, GL_DYNAMIC_DRAW);
    GLuint cubecolorbuffer;
    glGenBuffers(1, &cubecolorbuffer);
    glBindBuffer(GL_ARRAY_BUFFER, cubecolorbuffer);
    glBufferData(GL_ARRAY_BUFFER, sizeof(g_cube_color_buffer_data),
                 g_cube_color_buffer_data, GL_DYNAMIC_DRAW);
#endif
    ...
    do
    {
        ...
        cubeModelMatrix
            = glm::scale(cubeModelMatrix, glm::vec3(0.2f, 0.2f, 0.2f));
        ...
        // There should be a getCameraPosition() function in
        // common/controls.cpp, but this works too.
        ...
        // This is equivalent to mlutiplying (1,0,0) and (0,1,0) by
        // inverse(ViewMatrix). ViewMatrix is orthogonal (it was made this way),
        // so its inverse is also its transpose,
        // and transposing a matrix is "free" (inversing is slooow)
        glUniform3f(CameraRight_worldspace_ID, ViewMatrix[0][0],
                    ViewMatrix[1][0], ViewMatrix[2][0]);
        glUniform3f(CameraUp_worldspace_ID, ViewMatrix[0][1], ViewMatrix[1][1],
                    ViewMatrix[2][1]);

        glUniform3f(BillboardPosID, 0.0f, 0.5f,
                    0.0f);  // The billboard will be just above the cube
        glUniform2f(BillboardSizeID, 1.0f,
                    0.125f);  // and 1m*12cm, because it matches its 256*32
                              // resolution =)
        ...
        glEnableVertexAttribArray(0);
        glBindBuffer(GL_ARRAY_BUFFER, billboard_vertex_buffer);
        glVertexAttribPointer(0,  // attribute. No particular reason for 0, but
