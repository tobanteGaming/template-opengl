...
{
    // Load shaders
    ResourceManager::LoadShader(R"(shaders\sprite_vs.glsl)",
                                R"(shaders\sprite_fs.glsl)", nullptr, "sprite");
    ResourceManager::LoadShader(R"(shaders\particle_vs.glsl)",
                                R"(shaders\particle_fs.glsl)", nullptr,
                                "particle");
    // Configure shaders
    ...
    // Load textures
    ResourceManager::LoadTexture(R"(texture\background.jpg)", GL_FALSE,
                                 "background");
    ...
    ResourceManager::LoadTexture(R"(texture\block_solid.png)", GL_TRUE,
                                 "block_solid");
    ResourceManager::LoadTexture(R"(texture\paddle.png)", GL_TRUE, "paddle");
    ResourceManager::LoadTexture(R"(texture\particle.png)", GL_TRUE,
                                 "particle");
    ...
    Level three;
    Level four;
    one.Load(R"(levels\001.lvl)", m_width,
             static_cast<GLuint>(m_height * 0.5f));
    two.Load(R"(levels\002.lvl)", m_width,
             static_cast<GLuint>(m_height * 0.5f));
    three.Load(R"(levels\003.lvl)", m_width,
               static_cast<GLuint>(m_height * 0.5f));
    four.Load(R"(levels\004.lvl)", m_width,
              static_cast<GLuint>(m_height * 0.5f));
    ...
    m_player = std::make_unique<Entity>(playerPos, PLAYER_SIZE,
                                        ResourceManager::GetTexture("paddle"));

    // m_ball
    glm::vec2 ballPos
        = playerPos
          + glm::vec2(PLAYER_SIZE.x / 2 - BALL_RADIUS, -BALL_RADIUS * 2);
    ...
    for (Entity& box : m_levels[m_current_level].Bricks)
    {
        if (!box.Destroyed)
        {
            ...
            if (std::get<0>(collision))  // If collision is true
            {
                ...
                {
                    m_ball->Velocity.x
                        = -m_ball->Velocity.x;  // Reverse horizontal velocity
                    // Relocate
                    GLfloat penetration
                        = m_ball->Radius - std::abs(diff_vector.x);
                    if (dir == LEFT)
                    {
                        m_ball->Position.x
                ...
                {
                    m_ball->Velocity.y
                        = -m_ball->Velocity.y;  // Reverse vertical velocity
                    // Relocate
                    GLfloat penetration
                        = m_ball->Radius - std::abs(diff_vector.y);
                    if (dir == UP)
                    {
                        m_ball->Position.y
                            -= penetration;  // Move ball bback up
                    }
                    else
                    {
                        m_ball->Position.y
    ...
    if (!m_ball->Stuck && std::get<0>(result))
    {
        ...
        m_ball->Velocity
            = glm::normalize(m_ball->Velocity)
              * glm::length(
                  oldVelocity);  // Keep speed consistent over both axes
