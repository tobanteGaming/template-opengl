	...
	)
{
	...
	/*	If the user wants to activate the texture rectangle I kill a few flags	*/
	if( flags & SOIL_FLAG_TEXTURE_RECTANGLE )
	{
		...
		{
			/*	can't do it, and that is a breakable offense (uv coords activate pixels instead of [0,1]!)	*/
	...
	{
		/*	I've already made it a power of two, so simply activate the MIPmapping
	...
	/*	does the user want us to activate YCoCg color space?	*/
	...
	if( tex_id )
	{
		...
		if( flags & SOIL_FLAG_COMPRESS_TO_DXT )
		{
			...
			{
				/*	I can activate DXT, whether I compress it or OpenGL does	*/
				if( (channels & 1) == 1 )
				{
		...
		if( flags & SOIL_FLAG_MIPMAPS )
		{
			...
			/*	instruct OpenGL to activate the MIPmaps	*/
			glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
			glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		} else
		{
			/*	instruct OpenGL _NOT_ to activate the MIPmaps	*/
			glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
		...
		int loading_as_cubemap )
{
	...
	/*	got the image data RAM, create or activate an existing OpenGL texture handle	*/
	tex_ID = reuse_texture_ID;
	if( tex_ID == 0 )
	{
		glGenTextures( 1, &tex_ID );
	...
	{
		if( static_cast<int>(buffer_index + DDS_full_size) <= buffer_length )
		{
			...
			if( uncompressed )
			{
				...
				for(unsigned int j = 0; j < DDS_full_size; j += block_size )
				{
					unsigned char temp = DDS_data[j];
		...
		} else
		{
			...
			if( NULL == ext_addr )
			{
				/*	hmm, not good!!  This should not happen, but does on my
					...
					conversion, but I can't activate my own routines or load DDS files
